...
import { Mail, Lock, Eye, EyeOff, User, X, ShieldAlert, Loader2, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fc] text-slate-800 flex items-center justify-center p-4 md:p-8 font-sans transition-colors duration-300 max-md:bg-[#11163e] max-md:overflow-hidden max-md:relative">
      ...
      <div className="w-full max-w-4xl bg-white rounded-[32px] shadow-2xl flex overflow-hidden border border-slate-100 min-h-[600px] relative z-10 max-md:max-w-md max-md:p-0 max-md:border-none max-md:shadow-none max-md:bg-transparent">
        ...
        {/* Right Side: Signup Form (Form Modal Card on Mobile) */}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-start gap-2 text-xs pt-1">
                <input
                  type="checkbox"
                  required
                  className="w-4 h-4 text-indigo-600 border-slate-200 rounded focus:ring-indigo-500 cursor-pointer accent-indigo-600 mt-0.5 shrink-0"
                />
                <span className="text-slate-500 leading-normal">
                  I agree to the <span className="text-[#434bed] font-semibold hover:underline cursor-pointer">Terms of Service</span> and <span className="text-[#434bed] font-semibold hover:underline cursor-pointer">Privacy Policy</span>.
                </span>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3 bg-[#434bed] hover:bg-[#343cc7] text-white font-semibold text-sm rounded-2xl transition-all cursor-pointer shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-4"
              >
                {loading ? (
                  <>
